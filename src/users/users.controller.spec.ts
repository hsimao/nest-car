...
describe('UsersController', () => {
  ...
  it('findAllUsers returns a list of users with given email', async () => {
    const users = await controller.findAllUsers('mars@gmail.com');

    expect(users.length).toEqual(1);
    expect(users[0].email).toEqual('mars@gmail.com');
  });

  it('findUser returns a singel user with the given id', async () => {
